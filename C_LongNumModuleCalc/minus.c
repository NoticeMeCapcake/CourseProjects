...
int main()
{
	int *number1, number2[] = {1, 2, 3}, len1, len2 = 3, i;
	printf("len1: ");
	scanf("%d", &len1);
...
{
	int j = 0, i = 0, *buf, minus = 1, nul = 0, len_max = 0, less = 0;
	...
	if (len2 > len1 || less == 1)
    {
		...
		for (i = 0; i < len_max; i++) printf("buf00 = %d\n", buf[i]);
		free(number1);
        number1 = (int*) calloc(len2, sizeof(int));
        for (i = 0; i < len2; i++) number1[i] = buf[i];
		free(buf);
    }
    else if (len2 < len1 || less == 2)
    {
		...
		printf("baka1\n");
		free(number2);
        number2 = (int*) calloc(len1, sizeof(int));
        for (i = 0; i < len1; i++) number2[i] = buf[i];
		free(buf);
    }
	for (i = 0; i < len2; i++) printf("num10 = %d\n", number1[i]);
	for (i = 0; i < len2; i++) printf("num20 = %d\n", number2[i]);
	buf = (int*) calloc(len_max, sizeof(int));
	for (i = 0; i < len_max; i++) printf("buf0 = %d\n", buf[i]);
	for (i = len_max; i >= 0; i--)
	{
		if (less == 2)
		{
			buf[i] = number1[i] - number2[i];
			if (buf[i] < 0)
			{
				buf[i] += 10;
				number1[i - 1] -= 1;
	...
	for (i = 0; i < len_max; i++) printf("buf = %d\n", buf[i]);
	len1 = len_max - nul;
    j = len_max;
